...
When ants are exposed to different temperatures, lots of genes change expression. The number of responsive genes suggest that there is greater adaptability to cold than hot thermal stress. 

Comparisons between species indicate that the species from the warmer environment tolerates thermal stress by higher baseline expression of protective genes
...
The reaction norms of thermally-responsive genes between species differed by mean expression (*t*~`r t.mean$parameter`~ = `r t.mean$statistic`, *P* < `r max(t.mean$p.value, 0.001)`) which was about `r round(exp(t.mean$estimate))` TPM greater in *A. carolinensis* than *A picea*, and mean slope (*t*~`r t.slope$parameter`~ = `r t.slope$statistic`, *P* < `r max(t.slope$p.value, 0.001)`), which was about `r round(exp(t.mean$estimate))` $\Delta$TPM/degree greater in *A. picea*. Curvature (*t*~`r t.curvature$parameter`~ = `r t.curvature$statistic`, *P* < `r round(max(t.curvature$p.value, 0.001), 3)`) and wiggle (*t*~`r t.wiggle$parameter`~ = `r t.wiggle$statistic`, *P* < `r max(t.wiggle$p.value, 0.001)`) did not differ between species. Of the total differences ($\Delta T$) in reaction norms between species for each gene, about ...% (95% CI ... - ...%) and ...% (95% CI ... - ...%) were due to shifts in mean and slope, respectively. 

We predicted that we would observe that genes up-regulated in response to thermal stress would have increased basal expression in the other species. Instead, we found that for *High* genes 
*A. picea* (Wilcoxon V = `r w1$statistic`, *P* < `r max(w1$p.value, 0.001)`, 95% CI location shift `r w1$conf.int[1:2]`), *Low* genes in *A. carolinensis* (Wilcoxon V = `r w2$statistic`, *P* < `r max(w2$p.value, 0.001)`, 95% CI location shift `r w2$conf.int[1:2]`) and *Intermediate* genes in both (Wilcoxon V = `r w3$statistic`, *P* < `r max(w3$p.value, 0.001)`, 95% CI location shift `r w3$conf.int[1:2]`), basal expression near optimum temperatures of 19.5C was always higher in *A. carolinensis*. 

Overall greater thermal tolerance of one species would be indicated by finding that gene expression is stable across a greater portion of the thermal gradient. For *Bimodal* expressed genes, we found no difference (*t*~df=1406~ = 0.31, *P* = 0.76) between species in the variance of the reaction norms for thermally-responsive genes. In contrast, *Intermediate* genes from *A. carolinensis* had a broader thermal range of stable gene expression than those from *A. picea* (*t*~df=1406~ = 9.25, *P* < 0.001, 95% CI difference in means 0.41 - 0.64). In addition, the species differed in the temperatures at which thermally-responsive genes were activated. For genes with increased expression at *High* temperatures, we found that *A. carolinensis* genes were activated on average 1°C hotter than *A. picea* genes (Fig. 2). Similarly, for genes with increased expression at *Low* temperatures, *A. carolinensis* genes were activated on average 1°C hotter than *A. picea* genes (Fig. 2). Thus, while the activation temperatures differed, the size of the window between activation of *Low* and *High* genes remained the same between species, consistent with the results for *Bimodal* genes above.
...
overall pattern *A. carolinensis* 

- more genes thermally-responsive
- tolerates thermal stress by shutting down biological processes

overall pattern *A. picea*

- more *Low* genes

- greater adaptability to cold than hot thermal stress
    - *A. carolinensis* limited future potential to adapt to heat stress
- different mechanisms of thermal tolernace
    - *A. carolinensis* tolerates thermal stress by shutting down (*Intermediate* genes)
    - *A. picea* tolerates by a combination of shutting down and activating 'protective' genes 
       - some indication that *A. picea* maintains express in cold conditions that *A. carolinensis* shuts down based on overlap of *Low* genes in *A. picea* with *Intermediate* genes in *A. carolinensis*
- metabolic compensation?

-------------------------------------


Thermal tolerance is a key trait for determining the survival of individual organisms, the persistence of populations and the geographic limits of species. While considerable research has focused on a few key genes (e.g. heat shock proteins) involved in thermal stress response [...], little is known about the functional diversity or transcriptome-wide patterns of thermal responsiveness. In this study, we characterized the thermal reactionome of parapatric ant species, *Aphaenogaster picea* and *Aphaenogaster carolinensis* found in colder and warmer environments, respectively. We found that nearly 10% of the genes in this species show significant thermal responsiveness.
