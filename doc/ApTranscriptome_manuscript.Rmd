...
pandoc.table(tt3, style="rmarkdown", split.tables = Inf, caption = "Number of thermally-responsive genes in each expression category for each species. *AcNC* in rows and *ApVT* in columns.")
```
...
mosaicplot(t(tt2), shade = FALSE, ylab = "AcNC", xlab = "ApVT", main = "", cex = 0.5)
```
...
We quantified differences in the expression reaction norms of thermally-responsive genes between species as changes to due shifts in mean ($\Delta M$), slope ($\Delta S$), curvature ($\Delta C$) and wiggle ($\Delta W$). Overall, we found mean expression was greater in *AcNC* (*t*~`r t.offset$parameter`~ = `r t.offset$statistic`, *P* < `r max(t.offset$p.value, 0.001)`), 

but changes in the shape due to slope , curvature (*t* = 15.71, *P* < 0.001) and wiggle (*t* = -12.45, *P* < 0.001) were all highly significant. On average, slope (95% CI 0.41 - 0.48) and curvature (95% CI 0.25 - 0.33) were greater in *A. picea* than *A. carolinensis*, indicating a steeper response to temperature. 
