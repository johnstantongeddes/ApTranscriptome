...
Finally, the *genetic assimilation hypothesis* [@waddington1953; @sikkink2014] proposes that exposure to more extreme stressors selects for a shift from inducible to constitutive expression of stress-response genes. This hypothesis predicts that transcripts responsive to high temperatures in *A. picea* will have higher constitutive expression in *A. carolinensis*, whereas transcripts responsive to low temperatures in *A. carolinensis* will have higher constitutive expression in *A. picea*. Of course, these hypotheses are not mutually exclusive. By taking a reactionome approach, we can quantify if, and under what conditions, these mechanisms contribute to thermal adaptation.
...
The combined *Aphaenogaster* transcriptome assembly contained 98,186 transcripts. About half of these (51,246) transcripts had a signficant BLAST hit, of which 50% (25,797) had a top hit to Insecta and 37% (18,854) had a top hit to Formicidae. We identified `r prettyNum(sum(sigtable[sigtable$Coefficient %in% c("Temperature", "Temperature:Colony"), "Number.significant"]), big.mark = ",", scientific = FALSE)` transcripts, or 9% of the total, with expression patterns that were thermally responsive at a 5% False Discovery Rate [@benjamini1995]. Of these transcripts, `r round(sigtable[which(sigtable$Coefficient == "Temperature:Colony"), "Number.significant"] / sum(sigtable[sigtable$Coefficient %in% c("Temperature", "Temperature:Colony"), "Number.significant"]), 2) *100`% (`r prettyNum(sigtable[which(sigtable$Coefficient == "Temperature:Colony"), "Number.significant"], big.mark=",")`) had a non-linear relationship with temperature that would likely have been missed with a standard differential expression experiment (e.g. high vs. low temperature). The proportion of responsive transcripts is similar if we focus only on those transcript with a BLAST hit (`r prettyNum(responsive_transcripts_w_BLAST, big.mark = ",")` transcripts out of `r prettyNum(nrow(annotation.table.responsive), big.mark = ",")`). However, as with all *de novo* transcriptome assemblies, this assembly is fragmented due to partial contigs and alternative transcripts [@vijay2013] so this estimate is likely a lower bound for the true proportion of transcripts that are thermally responsive. 

Although the total number of thermally-responsive transcripts did not differ between species ($\chi^2$~`r chi2$parameter`~ = `r round(chi2$statistic, 2)`, *P* = `r round(chi2$p.value, 2)`) there was significant qualitative and quantitative variation in thermal responses among transcripts and between the two species. The majority of the responsive transcripts were in the **Low** category for both species (`r Ap.type.table.prop[2,"Low"]`% and `r Ap.type.table.prop[1,"Low"]`% for *A. carolinensis* and *A. picea*, respectively), followed by **Intermediate** (`r Ap.type.table.prop[2,"Intermediate"]`%), **High** (`r Ap.type.table.prop[2,"High"]`%),**Bimodal** (`r Ap.type.table.prop[2,"Bimodal"]`%) and **Not Responsive** (`r Ap.type.table.prop[2,"NotResp"]`%) for *A. carolinensis*, and **Bimodal** (`r Ap.type.table.prop[1,"Bimodal"]`%), **High** (`r Ap.type.table.prop[1, "High"]`%), **Intermediate** (`r Ap.type.table.prop[1,"Intermediate"]`%) and **Not Responsive** (`r Ap.type.table.prop[1,"NotResp"]`%) for *A. picea*. 

## Expression response to thermal extremes differs between species

The two species differed in the number of transcripts in each expression category (Table 1, $\chi^2$~`r chi1$parameter`~ = `r chi1$statistic`, *P* < `r max(chi1$p.value, 0.001)`). *Aphaenogaster picea* induced significantly more transcripts in response to both temperature extremes (**Bimodal** transcripts in Table 1; $\chi^2$~`r chi6$parameter`~ = `r chi6$statistic`, *P* < `r max(chi6$p.value, 0.001)`) than did *A. carolinensis*, which downregulated more transcripts under these conditions (**Intermediate** transcripts in Table 1; $\chi^2$~`r chi4$parameter`~ = `r chi4$statistic`, *P* < `r max(chi4$p.value, 0.001)`). Consistent with the *enhanced response* hypothesis, the cool-climate *A. picea* induced 1,146 (30%) more transcripts in response to low temperatures than the warm-climate *A. carolinensis* (**Low** transcripts in Table 1; $\chi^2$~`r chi3$parameter`~ = `r chi3$statistic`, *P* < `r max(chi3$p.value, 0.001)`). However, consistent with the predictions of the *tolerance hypothesis* but not the *enhanced response hypothesis*, at high temperatures *A. carolinensis* induced expression of ~14% fewer transcripts than *A. picea* (**High** transcripts in Table 1; $\chi^2$~`r chi5$parameter`~ = `r chi5$statistic`, *P* < `r max(chi5$p.value, 0.001)`). 
...
Illumina short-read sequence data is available from the NCBI Short Read Archive BioProject PRJNA260626. The Trinity transcriptome assembly, FastAnnotator annotation file and Sailfish gene expression quantification files are available at Harvard Forest Archives, HF-113 (http://harvardforest.fas.harvard.edu/data-archive) datasets hf113-38, hf113-39, and hf113-40 (doi:10.6073/pasta/050a6c7c2ff25841b070a0ecab32bc65). The reproducible and version-controlled script for data analysis are available on GitHub (https://github.com/johnstantongeddes/ApTranscriptome). 
