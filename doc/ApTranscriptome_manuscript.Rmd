...
to each transcript. Temperature and species were both fixed effects, with a quadratic term included for temperature. We used $log(TPM + 1)$ as the response to control for skew in the expression data. For a continuous predictor such as temperature, this regression approach is preferred to an ANOVA approach as it can reveal non-linear responses such as hump-shaped or threshold effects [@cottingham2005]. This method is robust to over-dispersion because we expect errors in the read count distribution [@anders2010] to be independent with respect to temperature. To correct for multiple testing, we calculated adjusted *P* values using the False Discovery Rate (FDR) approach of Benjamini and Hochberg [-@benjamini1995], retaining transcripts under the 5% FDR threshold. We evaluated the robustness of this approach by re-assigning each transcript within a species randomly to a temperature, fitting the polynomial model as above, calculating FDR and then determining how many transcripts were under the 5% FDR threshold for the resampled data, when we would expect there to be no significant transcripts. We repeated this resampling approach 100 times, and used the 95th quantile of false significant transcripts as the null expectation, only retaining the transcripts that were more significant from the real data. We identified thermally-responsive transcripts as the subset that had significant $\beta_2(temp)$, $\beta_3(temp^2)$, $\beta_4(species * temp)$ or $\beta_5(species * temp^2)$ terms after step-wise model selection by AIC. All analyses were performed with R 3.2 [@r_core_team2013] and are fully reproducible (Supporting Information). 
...
In addition, we also examined the specific patterns of shifts from one expression category to another between species. Expression patterns differed for `r prettyNum(sigtable[which(sigtable$Coefficient=="Temperature:Colony"), "Number.significant"], big.mark = ",")` (`r round(sigtable[which(sigtable$Coefficient=="Temperature:Colony"), "Number.significant"] / sigtable[which(sigtable$Coefficient=="Total"), "Number.significant"], 2) * 100`%) of the thermally responsive transcripts, most of which shifted from one expression category to another. These shifts in expression category between species were not randomly distributed (Stuart-Maxwell test of marginal homogeneity $\chi^2$~4~ = 1,259, P < 0.001, Fig. S1), indicating that these differences were unlikely to be due to neutral processes. 

The *enhanced response* and *tolerance* hypotheses make opposing predictions concerning the overlap in response patterns between the two species (Fig. 1). The *enhanced response hypothesis* posits that temperature adaptation uses existing mechanisms for thermal resistance, which should result in significant overlap in response and fewer transcripts shifting expression categories than expected by chance (Fig. 1, left). In contrast, the *tolerance hypothesis* predicts that transcripts involved in active defense will become non-responsive or shift to other expression categories in the better-adapted species (Fig. 1, right). We tested these predictions by testing if the transcripts upregulated in response to the temperature extreme experienced less frequently by each species (cool temperatures for the warm-climate *A. carolinensis*, and warm temperatures for the cool-climate *A. picea*) displayed the same response profile in the species that more frequently experiences those conditions.

Transcripts upregulated at low temperatures in *A. carolinensis* (**Low** and **Bimodal** transcripts) were less likely to be upregulated in any expression category in *A. picea* (Fig. 2A), with the exception of an excess of transcripts that shifted to the **High** category. This pattern is partially consistent with the *enhanced response* hypothesis, as the same transcripts are used to respond the the temperature extreme less frequently encountered in each species. In contrast, transcripts upregulated in response to high temperatures in *A. picea* (**High** and **Bimodal**) shifted expression categories in *A. carolinensis* (Fig. 2B), primarily to the **Intermediate** expression category (Fig. 2B). These transcripts are less likely to be upregulated in any context, consistent with the *tolerance hypothesis*.

## Molecular processes suggest a generalized stress response mechanism

The results of the gene set enrichment analysis revealed a number of gene groups enriched in each expression category. Across both species, there were 9 terms enriched in the **Bimodal** category, including terms such involved in stress response (regulation of cellular response to stress, signal transduction by p53 class mediator), cell death (apoptotic signaling pathway) and cellular organization (e.g. protein complex localization). The 6 terms enriched in the **Low** category suggest that proteins undergo structural (e.g protein acylation) and organizational (single-organism organelle organization) changes to tolerate colder temperatures, possibly to maintain membrane fluidity [@ohtsu1998]. In the **High** category had only a single enriched term, "nicotinamide metabolic process", while the **Intermediate** category had 5 terms including DNA packaging and metabolic process terms. 

## *A. carolinensis* has greater inertia of expression change to increases in temperature than does *A. picea*

As an additional test of the *tolerance hypothesis*, we examined the critical temperature of gene induction in response to increasing and decreasing temperatures. We compared between species the mean temperatures of transcript upregulation, defined as the temperature at which the transcript showed the greatest positive change in expression. In support of the *tolerance hypothesis* but not the *enhanced response hypothesis*, the temperature of induction at low temperatures was more extreme for *A. carolinensis* (`r round(T_on.low.ttest$estimate[1], 1)`°C) than *A. picea* (`r round(T_on.low.ttest$estimate[2], 1)`°C; *t*~`r prettyNum(T_on.low.ttest$parameter, digits=2)`~ = `r round(T_on.low.ttest$statistic, 1)`, *P* < `r round(max(T_on.low.ttest$p.value, 0.001), 3)`; Fig. 3A), though the temperature of induction did not differ between species for high temperatures (*t*~`r round(T_on.high.ttest$parameter, 0)`~ = `r round(T_on.high.ttest$statistic, 1)`, *P* < `r max(T_on.high.ttest$p.value, 0.001)`). 

Similarly, for down-regulated (**Intermediate**) transcripts, we compared the mean temperatures of downregulation of transcript expression between species at both high (> 20°C) and low (< 20°C) temperatures. As for the upregulated transcripts, *A. carolinensis* had greater inertia of gene expression in response to increasing temperatures. The temperature of downregulation for **Intermediate** transcripts was `r round(T_off.int.high.ttest$estimate[1], 1)`°C for *A. carolinensis* compared to `r round(T_off.int.high.ttest$estimate[2], 1)` for *A. picea* (*t*~`r round(T_off.int.high.ttest$parameter, 0)`~ = `r round(T_off.int.high.ttest$statistic, 1)`, *P* < `r round(max(T_off.int.high.ttest$p.value, 0.001), 3)`). The difference between temperatures was not significant with decreasing temperatures (*t*~`r round(T_off.int.low.ttest$parameter, 0)`~ = `r round(T_off.int.low.ttest$statistic, 1)`, *P* = `r round(max(T_off.int.low.ttest$p.value, 0.001), 3)`, Fig. 3B).


## No evidence for *genetic assimilation*
 
We tested the *genetic assimilation hypothesis* by comparing the log ratios of relative inducibility to relative baseline expression at the rearing temperature (25°C). If stress-response transcripts have shifted between species from inducible to constitutive expression, there should be a negative relationship between the two. We found no evidence of such a relationship for either temperature extreme: transcripts more upregulated at high temperatures in the cool-climate *A. picea* were not expressed at higher baseline levels in the warm-climate *A. carolinensis* (Fig. 4A). Similarly, transcripts more upregulated at low temperatures in *A. carolinensis* did not show higher baseline levels in *A. picea* (Fig. 4B). In fact, for both comparisons we found a weakly positive relationship between relative inducibility and baseline expression between the two species ($\beta_1$ = 0.31, *P* < 0.001 and ($\beta_1$ = 0.21, *P* < 0.001). In addition, the thermally responsive transcripts in *A. carolinensis*, regardless of expression pattern, had higher baseline expression than those in *A. picea*, including those with **Intermediate** expression profiles in both species (Wilcoxon V = `r prettyNum(w3$statistic)`, *P* < `r max(w3$p.value, 0.001)`). An important exception to this pattern is the set of transcripts that had **High** or **Bimodal** expression in *A. picea* but were not thermally responsive in *A. carolinensis* (far-right column of Fig. 2B). 
