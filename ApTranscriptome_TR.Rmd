...
grid.arrange(mh_plot_sub1, mh_plot_sub2, ncol=2)
dev.off()
...
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Low"), "value"] <- runif(5, min = -300, max = 0)
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Bimodal"), "value"] <- runif(5, min = -300, max = 0)
# change shared response to positive
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Low" & enhanced_defense_dat$Apic == "Low"), "value"] <- 500
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Bimodal" & enhanced_defense_dat$Apic == "Bimodal"), "value"] <- 300
...
# title
enhanced_defense_plot <- enhanced_defense_plot + ggtitle('Enhanced Response hypothesis') + theme(plot.title = element_text(hjust=0))
...
tolerance_dat[which(tolerance_dat$Acar == "Low"), "value"] <- runif(5, min = 100, max = 300)
tolerance_dat[which(tolerance_dat$Acar == "Bimodal"), "value"] <- runif(5, min = 100, max = 200)
# change shared response to near zero
tolerance_dat[which(tolerance_dat$Acar == "Low" & tolerance_dat$Apic == "Low"), "value"] <- 30
...
# title
tolerance_plot <- tolerance_plot + ggtitle('Tolerance hypothesis') + theme(plot.title = element_text(hjust=0))
tolerance_plot

# plot together
png("results/predictions_Fig1.png")
grid.arrange(enhanced_defense_plot, tolerance_plot, ncol=2)
dev.off()
```
...
A22.high.lm <- lm(log(A22.Ar.constitutive) ~ log(A22.Ar.upreg), data = Ap.response.type2.A22.high)
...
Ap.response.type <- as.data.frame(Ap.response.type)
