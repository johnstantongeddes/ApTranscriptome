...
Wow - even after controlling for FDR `r nrow(signif.transcripts)` transcipts are significant! 

Next, I use the draconianally-stringent Bonferonni correction. 

```{r bonferroni}
RxNpval$pbonf <- p.adjust(RxNpval$pval, method = "bonferroni")
```

Wow - `r length(which(RxNpval$pbonf < 0.05))` transcripts significant after Bonferroni correction!


```{r select_bonferroni}
# subset to significant transcripts
signif.transcripts <- RxNpval[which(RxNpval$pbonf < 0.05), ]
...
Contrary to expectations, genes upregulated at high temperatures in *A22* also have greater expression at 19.5°C than *Ar*. 

Next I test the converse, that transcripts that are up-regulated at low temperatures in *Ar* are more highly-expressed at 19.25°C temperatures in *A22*. 
...
Consistent with expectations, and the above test, expression at 19.5°C is also greater in *A22* than *Ar* for transcripts upregulated at low temperatures in *Ar*. 
...
boxplot(log(Ar.int.transcripts$A22.opt+1), log(Ar.int.transcripts$Ar.opt+1))
# Wilcoxon signed rank-test
...
boxplot(log(A22.int.transcripts$A22.opt+1), log(A22.int.transcripts$Ar.opt+1))
# Wilcoxon signed rank-test
wilcox.test(A22.int.transcripts$A22.opt, A22.int.transcripts$Ar.opt, alternative = "two.sided", paired = TRUE, conf.int = TRUE)
```

The non-parametric test for both comparisions also finds greater constitutive expression in *A22* than *Ar*.

The overall pattern that emerges from these 4 tests is that constitutive expression at optimal conditions of 19.5°C is overall greater for *A22* than *Ar*.
