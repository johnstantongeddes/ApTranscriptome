...
library(MASS)
library(xtable)
library(methods) # required by `knitcitations` when file is `knit` file from command-line
...
DeconSeq identified contaminant genes. I remove this n
...
pandoc.table(tt3, style="rmarkdown", caption = "Number of transcripts with maximum expression at high, low, intermediate, both high and low (bimodal) temperatures or are not thermally-responsivefor each colony and their overlap.", split.tables = "Inf")
```
...
t.mean <- t.test(log(varshape.out$A22.mean), log(varshape.out$Ar.mean))
...
# t-test
(t.offset <- t.test(varshape.out$s.offset, alternative = "two.sided"))

# slope = slope1 - slope2
varshape.out$slope <- varshape.out$A22.slope - varshape.out$Ar.slope

# t-test
(t.slope <- t.test(varshape.out$s.slope, alternative = "two.sided"))
...
# t-test
(t.curvature <- t.test(varshape.out$s.curvature, alternative = "two.sided"))
...
# note small number of genes with 'wiggle'
length(which(varshape.out$s.wiggle != 0))

# t-test
(t.wiggle <- t.test(varshape.out$s.wiggle, alternative = "two.sided"))
```
...
qwiggle <- quantile(varshape.out$prop.wiggle, probs = c(0.05, 0.5, 0.95))
```
...
Calculate T~on~ for *High* genes in each colony.
...
Genes with increased expression at *High* temperatures are turned on at, on average, 1°C higher in *AcNC* than *ApVT*.
...
Genes with increased expression at *High* temperatures are turned on at, on average, 1°C higher in *ApVT* than *AcNC*.
...
```{r gsea_intermediate, results = 'hide'}
...
save(RxNpval, sigtable, chi1, resp.TPM.dt.sub, tt2, tt3, GO.stress.Xsq, GO0006950.responsive, Ap.df, w1, w2, Ap.T_on, Ap.BP.resultParentChild, Ap.gsea1, Ap.setdiff.gsea, resp.stress.responsive.count, t.offset, t.slope, t.curvature, t.wiggle, file = "ms.RData")
```
