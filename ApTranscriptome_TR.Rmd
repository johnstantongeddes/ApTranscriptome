...
The previous section identified the transcripts with thermally-responsive expression. In this section, I determine the shape of the expression response to temperature for each transcript. Categories of expression response are:
...
response.type <- ldply(interaction.transcripts, .progress = "text", .inform = TRUE, RxNtype)




# plot
#qplot(val, pTPM, facets = .~colony, data = A22.pout, geom="smooth")



# merge RxN results with expression values
temperature.transcripts.TPM <- TPM.dt.sub[names(temperature.transcripts)]
...
temperature.transcripts.type$A22.exp.type <- as.factor(temperature.transcripts.type$A22.exp.type)
...
A22.interaction.transcripts.type$A22.exp.type <- as.factor(A22.interaction.transcripts.type$A22.exp.type)
...
Ar.interaction.transcripts.type$A22.exp.type <- as.factor(Ar.interaction.transcripts.type$A22.exp.type)
...
```{r expression_A22.exp.type_table, eval=TRUE, echo=FALSE, results='asis'}
# table type of response
A22.type.table <- table(responsive.transcripts.type[responsive.transcripts.type$colony == "A22", 'A22.exp.type'])
Ar.type.table <- table(responsive.transcripts.type[responsive.transcripts.type$colony == "Ar", 'A22.exp.type'])

A22.exp.type.table <- rbind(A22.type.table, Ar.type.table)
rownames(A22.exp.type.table) <- c("A22", "Ar")

pandoc.table(A22.exp.type.table, style="rmarkdown", caption = "Number of transcripts with maximum expression at high, low, intermediate or both high and low (bimodal) temperatures.")
```
...
A22_high_transcripts <- responsive.transcripts.type[which(responsive.transcripts.type$colony == "A22" & responsive.transcripts.type$A22.exp.type == "High"), "Transcript"]
...
Ar_low_transcripts <- responsive.transcripts.type[which(responsive.transcripts.type$colony == "Ar" & responsive.transcripts.type$A22.exp.type == "Low"), "Transcript"]
...
A22_int_transcripts <- responsive.transcripts.type[which(responsive.transcripts.type$colony == "A22" & responsive.transcripts.type$A22.exp.type == "Intermediate"), "Transcript"]
...
Ar_int_transcripts <- responsive.transcripts.type[which(responsive.transcripts.type$colony == "Ar" & responsive.transcripts.type$A22.exp.type == "Intermediate"), "Transcript"]
...
A22trlist <- A22.interaction.transcripts.type[which(A22.interaction.transcripts.type$A22.exp.type == "Intermediate"), "Transcript"]
...
# repeat for Ar
Artrlist <- Ar.interaction.transcripts.type[which(Ar.interaction.transcripts.type$A22.exp.type == "Intermediate"), "Transcript"]
...
A22trlist.bim <- A22.interaction.transcripts.type[which(A22.interaction.transcripts.type$A22.exp.type == "Bimodal"), "Transcript"]
...
# repeat for Ar
Artrlist.bim <- Ar.interaction.transcripts.type[which(Ar.interaction.transcripts.type$A22.exp.type == "Bimodal"), "Transcript"]
...
g5 + facet_grid(. ~ A22.exp.type) + scale_y_continuous(name="Density") +
  scale_x_continuous(name="Temperature of maximum expression")
dev.off()
```
