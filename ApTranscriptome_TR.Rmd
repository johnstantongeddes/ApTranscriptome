...
Preliminary runs of Sailfish revealed that while quantification is highly consistent among runs, small stochastic differences impact which transcripts are found to be thermally-responsive. To address this, I repeated that expression quantification 10 times and retain only the transcripts that are responsive in at least 8 of to 10 replicates.

The gene expression replicates are generated by running the `scripts/sailfish_rep.sh` script, which uses GNU parallel to repeat the `scripts/sailfish_quant.R` script which runs Sailfish for each of the 24 samples.
...
# replicates
reps <- c("rep1", "rep2", "rep3", "rep4", "rep5", "rep6", "rep7", "rep8", "rep9", "rep10")

# loop across replicates and load expression data
for(sr in reps) {
  read.sailfish.reps(sr, fd=sfexpressionroot, repout=paste("TPM.dt.", sr, sep=""))
}
```

For any single replicate, the expression levels at each temperature treatment are highly correlated between the two colonies.

```{r exp_correlations, echo=FALSE, results='hide'}
cortable <- ddply(TPM.dt.rep1, .(val), summarize, cor(TPM[which(colony=="A22")], TPM[which(colony=="Ar")]))
```
...
# setkey of each TPM replicate to 'Transcript'
ls1 <- ls(pattern="TPM.dt.rep")
sapply(mget(ls1), function(x) setkey(x, Transcript))
sapply(mget(ls1), function(x) key(x))
# remove Transcripts found in contaminant list
TPM.dt.rep1 <- TPM.dt.rep1[!cont.list]
TPM.dt.rep2 <- TPM.dt.rep2[!cont.list]
TPM.dt.rep3 <- TPM.dt.rep3[!cont.list]
TPM.dt.rep4 <- TPM.dt.rep4[!cont.list]
TPM.dt.rep5 <- TPM.dt.rep5[!cont.list]
TPM.dt.rep6 <- TPM.dt.rep6[!cont.list]
TPM.dt.rep7 <- TPM.dt.rep7[!cont.list]
TPM.dt.rep8 <- TPM.dt.rep8[!cont.list]
TPM.dt.rep9 <- TPM.dt.rep9[!cont.list]
TPM.dt.rep10 <- TPM.dt.rep10[!cont.list]

# remove contaminants from annotation.table
...
RxNpval.r1 <- ddply(TPM.dt.rep1, .(Transcript), .inform="TRUE", modpFunc)
RxNpval.r2 <- ddply(TPM.dt.rep2, .(Transcript), .inform="TRUE", modpFunc)
RxNpval.r3 <- ddply(TPM.dt.rep3, .(Transcript), .inform="TRUE", modpFunc)
RxNpval.r4 <- ddply(TPM.dt.rep4, .(Transcript), .inform="TRUE", modpFunc)
...
```{r padjust}
RxNpval.r1$padj <- p.adjust(RxNpval.r1$pval, method = "fdr")
RxNpval.r2$padj <- p.adjust(RxNpval.r2$pval, method = "fdr")
RxNpval.r3$padj <- p.adjust(RxNpval.r3$pval, method = "fdr")
RxNpval.r4$padj <- p.adjust(RxNpval.r4$pval, method = "fdr")
...
signif.transcripts.r1 <- RxNpval.r1[which(RxNpval.r1$padj < 0.05), ]
signif.transcripts.r2 <- RxNpval.r2[which(RxNpval.r2$padj < 0.05), ]
signif.transcripts.r3 <- RxNpval.r3[which(RxNpval.r3$padj < 0.05), ]
signif.transcripts.r4 <- RxNpval.r4[which(RxNpval.r4$padj < 0.05), ]
...
```{r plot_GOstress, echo=FALSE, eval=FALSE}
...
save(RxNpval, sigtable, chi1, resp.TPM.dt.sub, Ap.type.table, tt2, tt3, qlslope, qlmean, GO.stress.Xsq, GO0006950.responsive, Ap.df, w1, w2, w3, Ap.T_on, Ap.BP.resultParentChild, resp.stress.responsive.count, t.mean, t.slope, t.curvature, t.wiggle, t.varint, t.varbim, T_on.high.ttest, T_on.low.ttest, Ap.gsea.intersect, Ap.gsea.union, Ar.int.A22.bim.gsea, Ar.int.A22.low.gsea, file = "ms.RData")
```
