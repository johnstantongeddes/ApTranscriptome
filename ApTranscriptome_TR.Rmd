...
We quantify the relative importance of each of these mechanisms to thermal adaptation in these two species by evalutating differences in the reactionomes between species. We use gene set enrichment analysis to provide additional insight on the cellular processes underlying the transcriptome-wide changes in expression. 
...
# plot
enhanced_response_plot <- qplot(y=Apic, x=Acar, data=enhanced_response_dat, fill=value, geom="tile", xlim = rev(levels(enhanced_response_dat$Acar))) + 
...
Next, I checked the same for the **Bimodal** genes in *A. picea*.
...
As with the **High** transcripts, expression was much higher for the responsive *A. picea* transcripts than for the non-responsive *A. carolinensis* transcripts.
...
To assist visualization of specific transcripts, I made a interactive web-app using the [shiny](http://www.rstudio.com/shiny/) package. The scripts for this app are in the sub-directory `.\ApRxN-shinyapp`. The web app is available at https://johnsg.shinyapps.io/ApRxN-shinyapp/
...
save(sigtable, chi1, chi2, chi3, chi4, chi5, chi6, resp.TPM.dt.sub, Ap.type.table, Ap.type.table.prop, tt2, tt3, GO.stress.Xsq, GO0006950.responsive, Ap.df, w1, w2, w3, hsp_responsive, hsp_all, mh.test, mh_plot, T_on.high.ttest, T_on.low.ttest, T_off.int.high.ttest, T_off.int.low.ttest, Ar.int.A22.bim.gsea, Ar.int.A22.low.gsea, Ar.int.A22.lowbim.gsea, md, responsive_transcripts_w_BLAST, annotation.table.responsive, file = paste("doc/ms_slim.RData", sep = ""))
```
