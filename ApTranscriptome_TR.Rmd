...
md_sub1 <- subset(md, Acar %in% c("Low", "Bimodal"), )
...
mh_plot_sub2 <- qplot(x=Apic, y=Acar, data=md_sub2, fill=value, geom="tile", ylim = rev(levels(md_sub2$Acar))) + 
  scale_fill_gradient2(limits=c(-600, 600)) +
  theme(axis.title = element_text(face="italic")) +
  labs(x = "A. picea", y = "A. carolinensis") + 
  theme(legend.position = "none")
...
# get legend from http://stackoverflow.com/questions/11883844/inserting-a-table-under-the-legend-in-a-ggplot2-histogram
#Extract Legend
...
legend <- g_legend(mh_plot)

# remove legend from mh_plot
mh_plot <- mh_plot + theme(legend.position = "none")
...
grid.arrange(mh_plot, mh_plot_sub2, mh_plot_sub1, legend,
             ncol=2, widths=c(1,0.5), heights=c(1, 0.5))

arrangeGrob(mh_plot, mh_plot_sub1, mh_plot_sub2, ncol=1), 
...
To visualize this data, I plot the log ratio of inducibility between species against the log ratio of baseline expression between species. I do this separately for **High** transcripts in *A. picea* and then **Low** transcripts in *A. carolinensis*. 
...
I extract the **High** transcripts for *A. picea* and plot the log ratio of inducibility in *A. picea* over inducibility in *A. carolinensis*, against the log ratio of baseline expression in *A. carolinensis* against expression in *A. picea*. According to the *genetic assimilation hypothesis* there should be a positive relationship between these ratios. That is, greater baseline expression in *A. carolinensis* should have greater inducibility towards **High** temperatures in *A. picea*.
...
gg11 <- ggplot(Ap.response.type2.A22.high, aes(y = log(A22.Ar.upreg+0.01), x = log(Ar.A22.baseline))) + 
  ...
  labs(y = expression(paste("log(inducibility ", italic("A. picea / A. carolinensis"), ")")),
       x = expression(paste("log(baseline ", italic("A. carolinensis / A. picea"), ")")))    
gg11

A22.high.lm <- lm(log(A22.Ar.upreg) ~ log(Ar.A22.baseline), data = Ap.response.type2.A22.high)

summary(A22.high.lm)
```

In contrast to the prediction there is a negative relationship between these ratios. That is, genes with greater upregulation of expression also have greater baseline expression. This is also consistent with the results simply comparing baseline expression between species for the *A. picea* **High** transcripts. 
...
gg12 <- ggplot(Ap.response.type2.Ar.low, aes(y = log(Ar.A22.upreg), x = log(A22.Ar.baseline))) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  labs(y = expression(paste("log(inducibility ", italic("A. carolinensis / A. picea"), ")")),
       x = expression(paste("log(baseline ", italic("A. picea / A. carolinensis"), ")")))                   

gg12

Ar.low.lm <- lm(log(Ar.A22.upreg) ~ log(A22.Ar.baseline), data = Ap.response.type2.Ar.low)
...
```{r fig3}
# grid.arrange
png(file = "results/genetic_assimilation.png")
