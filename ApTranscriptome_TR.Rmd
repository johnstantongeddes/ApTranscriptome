...
tiff("results/matched_observations_Fig1.tiff", compression = "lzw")
mh_plot
dev.off()
```
...
### Comparison of the degree of upregulation against constitutive expression

To test the genetic assimilation hypothesis, I compare the degree of upregulation of a transcript against constitutive expression at optimal temperatures. 
...
# plot constitutive expression vs induction for High, Low and Bimodal genes only
setkey(Ap.response.type2, A22.type)
up.terms <- c("Bimodal", "Low", "High")
Ap.response.type2.up <- Ap.response.type2[A22.type %in% up.terms, ]

setkey(Ap.response.type2.up, Ar.type)
Ap.response.type2.up <- Ap.response.type2.up[Ar.type %in% up.terms, ]
Ap.response.type2.up <- droplevels(Ap.response.type2.up)

gg5 <- ggplot(Ap.response.type2.up, aes(x = log(Ar.opt), y = log(A22.upreg), colour = A22.type)) + 
  geom_point() + 
  geom_smooth() +
  labs(x="log(consitutive expression of A. carolinensis)", y = "log(Degree of inducibility in A. picea)") +
  theme(legend.position = "none")  

gg6 <- ggplot(Ap.response.type2.up, aes(x = log(A22.opt), y = log(Ar.upreg), colour = Ar.type)) + 
  geom_point() + 
  geom_smooth() +
  labs(x="log(consitutive expression of A. picea)", y = "log(Degree of inducibility in A. carolinensis)") +
  theme(legend.justification=c(1,0), legend.position=c(1,0), legend.text = element_text(size = 12))  

# grid.arrange
png("results/constitutive_vs_inducibility.png")
grid.arrange(gg5, gg6, nrow = 1)
dev.off()

### plot constitutive expression in one species vs the other
gg7 <- ggplot(Ap.response.type2.up, aes(x = log(Ar.opt), y = log(A22.opt), colour = A22.type)) + 
  geom_point() + 
  geom_smooth() +
  labs(x="log(consitutive expression of A. carolinensis)", y = "log(constitutive expression in A. picea)") +
  theme(legend.position = "none")  
gg7 <- gg7 + geom_abline(intercept=0, slope=1)

png("results/constitutive_comparison.png")
gg7
dev.off()

### plot inducibility in one species vs the other
gg8 <- ggplot(Ap.response.type2.up, aes(x = log(Ar.upreg), y = log(A22.upreg), colour = A22.type)) + 
  geom_point() + 
  geom_smooth() +
  labs(x="log(upregulation in A. carolinensis)", y = "log(upregulation in A. picea)") +
  theme(legend.position = "none")  
gg8

png("results/upregulation_comparison.png")
gg8
dev.off()

### constitutive vs upregulated for *High* genes only
...
gg9 <- ggplot(Ap.response.type2.A22.high, aes(x = log(Ar.opt), y = log(A22.opt), colour = Ar.type)) + 
  geom_point() + 
  geom_smooth() +
  labs(x="log(consitutive expression of A. carolinensis)", y = "log(Degree of inducibility in A. picea)") +
  theme(legend.position = "none")  
gg9 + geom_abline(intercept=0, slope=1)

png("results/constitutive_vs_inducibility_A22high.png")
gg9
dev.off()


Ar.high.transcripts.df <- Ap.response.type[which(Ap.response.type$Ar.type == "High"), ]
Ap.response.type2.Ar.high <- Ap.response.type2[Ap.response.type2$Transcript %in% Ar.high.transcripts.df$Transcript, ]
str(Ap.response.type2.Ar.high)

gg10 <- ggplot(Ap.response.type2.Ar.high, aes(x = log(A22.opt), y = log(Ar.opt), colour = A22.type)) + 
  geom_point() + 
  geom_smooth() +
  labs(x="log(consitutive expression of A. picea)", y = "log(Degree of inducibility in A. carolinensis)") 
...
gg10 + geom_abline(intercept=0, slope=1)
```


```{r log_inducibility}
str(Ar.high.transcripts.df)
Ap.response.type2.A22.high <- Ap.response.type2[Ap.response.type2$Transcript %in% Ar.high.transcripts.df$Transcript, ]
str(Ap.response.type2.A22.high)

# exclude transcripts with zero upregulation for denominator
Ap.response.type2.A22.high <- Ap.response.type2.A22.high[which(Ap.response.type2.A22.high$Ar.upreg != 0), ]

Ap.response.type2.A22.high$A22.Ar.upreg <- Ap.response.type2.A22.high$A22.upreg / Ap.response.type2.A22.high$Ar.upreg

# exclude transcripts with zero ratio of upregulation
Ap.response.type2.A22.high <- Ap.response.type2.A22.high[which(Ap.response.type2.A22.high$A22.Ar.upreg != 0), ]

# ratio of baseline expression
Ap.response.type2.A22.high$Ar.A22.baseline <- Ap.response.type2.A22.high$Ar.opt / Ap.response.type2.A22.high$A22.opt

head(Ap.response.type2.A22.high)

# plot!
gg11 <- ggplot(Ap.response.type2.A22.high, aes(y = log(A22.Ar.upreg+0.01), x = log(Ar.A22.baseline))) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  labs(y="log(inducibility A. picea / inducibility A. car)", x = "log(baseline A. carolinensis / baseline A. picea)") 


summary(lm(log(A22.Ar.upreg) ~ log(Ar.A22.baseline), data = Ap.response.type2.A22.high))

cor.test(log(Ap.response.type2.A22.high$A22.Ar.upreg), log(Ap.response.type2.A22.high$Ar.A22.baseline))

### Repeat for A. carolinensis *Low* transcripts

str(Ar.low.transcripts.df)
...
# exclude transcripts with zero ratio of upregulation
Ap.response.type2.Ar.low <- Ap.response.type2.Ar.low[which(Ap.response.type2.Ar.low$Ar.A22.upreg != 0), ]

Ap.response.type2.Ar.low$A22.Ar.baseline <- Ap.response.type2.Ar.low$A22.opt / Ap.response.type2.Ar.low$Ar.opt

head(Ap.response.type2.Ar.low)

gg12 <- ggplot(Ap.response.type2.Ar.low, aes(y = log(Ar.A22.upreg), x = log(A22.Ar.baseline))) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  labs(y="log(inducibility A. car / inducibility A. picea)", x = "log(baseline A. picea / baseline A. car)") 
...
As with genes turned on at high temperatures, *A. carolinensis* **Intermediate** genes are down-regulated on average at 2.7°C higher temperatures than *A. picea* **Intermediate** genes. In contrast, towards low temperatures, the temperature of mean temperature of downregulation was higher for *A. picea* (10.6°) than *A. carolinensis* (9.75°).
...
# grid.arrange
#tiff("results/crit_temp_regulation_Fig2.tiff", compression = "lzw", bg = "white")
