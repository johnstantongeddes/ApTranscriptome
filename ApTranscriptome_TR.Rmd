...
# calculate overall P value for 'model' for each transcript
...
# apply predFunc to responsive transcripts
testpred <- ddply(resp.TPM.dt.sub[1:220], .(Transcript), .inform="TRUE", predFunc)

# calculate temperature of greatest change
T.on.df <- ddply(testpred, .(Transcript, colony), summarise, 
              T_on <- diff(pTPM))


testdiff[(which(diff(testdiff$pTPM) == max(diff(testdiff$pTPM)))+1), ]
