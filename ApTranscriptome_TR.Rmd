...
interaction.transcripts <- RxNlmAIC[which(Map(grepFunc, RxNlmAIC, term = "colonyAr:") == TRUE)]
other.transcripts <- RxNlmAIC[setdiff(names(RxNlmAIC), names(interaction.transcripts))]
temperature.transcripts <- other.transcripts[which(Map(grepFunc, other.transcripts, term = "val") == TRUE)]
colony.transcripts <- other.transcripts[setdiff(names(other.transcripts), names(temperature.transcripts))]
responsive.transcripts <- c(temperature.transcripts, interaction.transcripts)
rm(other.transcripts)
```
...
siglist <- c(length(RxNlmAIC),
             length(colony.transcripts),
             length(temperature.transcripts),
             length(interaction.transcripts)
             )
...
interaction.response.type <- ldply(interaction.transcripts, .progress = "text", .inform = TRUE, RxNtype)
stopifnot(nrow(interaction.response.type) == length(interaction.transcripts))

# calculate response types for transcripts without interactions
temperature.response.type <- ldply(temperature.transcripts, .progress = "text", .inform = TRUE, RxNtype)
stopifnot(nrow(temperature.response.type) == length(temperature.transcripts))
...
# overlap
overlap <- c(length(Ap.response.type[which(Ap.response.type$A22.type == "Bimodal" & Ap.response.type$Ar.type == "Bimodal"), "Transcript"]),
             length(Ap.response.type[which(Ap.response.type$A22.type == "High" & Ap.response.type$Ar.type == "High"), "Transcript"]),
             length(Ap.response.type[which(Ap.response.type$A22.type == "Intermediate" & Ap.response.type$Ar.type == "Intermediate"), "Transcript"]),
             length(Ap.response.type[which(Ap.response.type$A22.type == "Low" & Ap.response.type$Ar.type == "Low"), "Transcript"]),
             length(Ap.response.type[which(Ap.response.type$A22.type == "NotResp" & Ap.response.type$Ar.type == "NotResp"), "Transcript"]))

# make table
Ap.type.table <- rbind(A22.type.table, Ar.type.table, overlap)
rownames(Ap.type.table) <- c("ApVT", "ApNC", "Overlap")

# re-orderfor printing
Ap.type.table <- Ap.type.table[ , c(2,4,1,3,5)]
...
shiny.transcripts <- TPM.dt.sub[Ap.response.type$Transcript]

# scale expression values 
shiny.transcripts[,pTPM.scaled:=scale(pTPM), by = Transcript]
...
```{r A22_high_wilcoxon}
wilcox.test(A22.high.transcripts$A22.opt, A22.high.transcripts$Ar.opt, alternative = "two.sided", paired = TRUE, conf.int = TRUE)
```
...
wilcox.test(Ar.low.transcripts$A22.opt, Ar.low.transcripts$Ar.opt, alternative = "two.sided", paired = TRUE, conf.int = TRUE)
```
...
t1 <- t.test(Ar.int.sd$exp_sd, A22.int.sd$exp_sd)
```
...
g3 <- ggplot(Ap.df, aes(x=max.val, fill=colony)) + geom_density(alpha=0.2, position="identity")
...
varshape.out <- ldply(interaction.transcripts, .progress = "text", RxNvarshape)
...
# offset = mean1 - mean2
varshape.out$offset
<- varshape.out$A22.mean - varshape.out$Ar.mean
...
responsive.transcripts.ann.type <- signif.transcripts[names(responsive.transcripts)]
# combine responsive.transcripts.ann with "type" information
responsive.transcripts.ann.type <- merge(responsive.transcripts.ann, Ap.response.type[,c("Transcript", "A22.type", "Ar.type")], by = "Transcript", all = TRUE)
str(responsive.transcripts.ann.type)
write.csv(responsive.transcripts.ann.type, file = "results/Ap_responsive_genes_20140409.csv", row.names = FALSE)
```
...
GO0006950.responsive <- responsive.transcripts.ann.type[grep("GO:0006950", responsive.transcripts.ann.type$GO.Biological.Process), list(Transcript, best.hit.to.nr, A22.type, Ar.type)]
...
# Chi-square test to see if 'response to stress' related genes overrepresented in responsive.transcripts compared to full list
resp.stress.responsive.count <- nrow(responsive.transcripts.ann.type[grep("GO:0006950", responsive.transcripts.ann.type$GO.Biological.Process), list(Transcript, best.hit.to.nr)])
...
GO.stress.table <- matrix(rbind(resp.stress.responsive.count, nrow(responsive.transcripts.ann.type) - resp.stress.responsive.count, resp.stress.all.count, nrow(annotationtable) - resp.stress.all.count), nrow = 2)
...
```{r grep_shock}
hsp_responsive <- responsive.transcripts.ann.type[grep("shock", responsive.transcripts.ann.type$best.hit.to.nr), list(Transcript, best.hit.to.nr, A22.type, Ar.type)]
hsp_responsive
...
# table results
Ap.BP.ResTable <- GenTable(Ap.BP.GOdata, parentchild = Ap.BP.resultParentChild, topNodes = 82)
dim(Ap.BP.ResTable)
...
# Run GSEA
Ap.high.gsea <- gsea(genelist = Ap.geneList.high, geneID2GO = geneID2GO, plotpath = "results/Ar_int_BP_topGO_sig_nodes.pdf")
...
p3 <- ggplot(temperature.transcripts.TPM, aes(x=val, y=exp.scaled, group=Transcript)) +
  geom_line() +
  facet_grid(A22.exp.type ~ colony) +
  scale_y_continuous(name="Expression (scaled)") +
  scale_x_continuous(name=expression(paste("Temperature ", degree, "C")))
p3
...
save(Ap.type.table, Ap.df, file = "ms.RData")
```
