...
md_sub1 <- subset(md, Acar %in% c("Low", "Bimodal", "High"), )
...
mh_plot_sub1 <- qplot(x=Apic, y=Acar, data=md_sub1, fill=value, geom="tile", ylim = rev(levels(md_sub1$Acar))) + 
  ...
  geom_rect(xmin = 0.5, xmax = 1.5, ymin = 2.5, ymax = 3.5, fill = "transparent", linetype=2, colour="black") +
  geom_rect(xmin = 2.5, xmax = 3.5, ymin = 1.5, ymax = 2.5, fill = "transparent", linetype=2, colour="black") +
  geom_rect(xmin = 3.5, xmax = 4.5, ymin = .5, ymax = 1.5, fill = "transparent", linetype=2, colour="black")

mh_plot_sub1

md_sub2 <- subset(md, Apic %in% c("Low", "High", "Bimodal"), )
...
mh_plot_sub2 <- mh_plot_sub2 + ggtitle('A') + theme(plot.title = element_text(hjust=0))
mh_plot_sub1 <- mh_plot_sub1 + ggtitle('B') + theme(plot.title = element_text(hjust=0))

png("results/matched_observations_Fig1.png")
grid.arrange(mh_plot_sub2, mh_plot_sub1, ncol=1, heights = c(0.45, 0.6))
dev.off()
...
Ap.response.type2.A22.high$Ar.A22.constitutive <- Ap.response.type2.A22.high$Ar.opt / Ap.response.type2.A22.high$A22.opt

# plot!
gg11 <- ggplot(Ap.response.type2.A22.high, aes(y = log(A22.Ar.upreg), x = log(Ar.A22.constitutive))) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  labs(y = expression(paste("log(inducibility ", italic("A. picea / A. carolinensis"), ")")),
       x = expression(paste("log(constitutive ", italic("A. carolinensis / A. picea"), ")")))    

gg11

A22.high.lm <- lm(log(A22.Ar.upreg) ~ log(Ar.A22.constitutive), data = Ap.response.type2.A22.high)
...
# ratio of constitutive expression
Ap.response.type2.Ar.low$A22.Ar.constitutive <- Ap.response.type2.Ar.low$A22.opt / Ap.response.type2.Ar.low$Ar.opt

gg12 <- ggplot(Ap.response.type2.Ar.low, aes(y = log(Ar.A22.upreg), x = log(A22.Ar.constitutive))) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  labs(y = expression(paste("log(inducibility ", italic("A. carolinensis / A. picea"), ")")),
       x = expression(paste("log(constitutive ", italic("A. picea / A. carolinensis"), ")")))                   

gg12

Ar.low.lm <- lm(log(Ar.A22.upreg) ~ log(A22.Ar.constitutive), data = Ap.response.type2.Ar.low)
...
# grid.arrange
png(file = "results/genetic_assimilation_Fig2.png")
