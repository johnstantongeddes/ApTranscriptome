...
In this technical report, which accompanies the manuscript **Thermal reactionomes reveal divergent responses to thermal extremes in warm and cool-climate ant species** (Stanton-Geddes et al., in press), we:
...
3. Evaluate differences in transcriptome-wide patterns of thermal responsiveness (i.e. the thermal reactionome) between the two species
...
# for subsequent analyses, focus on the transcripts with significant temperature x species interaction only
...
write.table(file = paste(resultsdir, "Ap_responsive_transcripts_", Sys.Date(), ".txt", sep = ""), interaction.response.type, row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
```
...
A22.type.table <- table(interaction.response.type$A22.type)
Ar.type.table <- table(interaction.response.type$Ar.type)
...
type.table <- table(Acar = interaction.response.type$Ar.type, Apic = interaction.response.type$A22.type)
# reorder
...
mh_plot <- qplot(x=Apic, y=Acar, data=md, fill=value, geom="tile", ylim = rev(levels(md$Acar))) +
  scale_fill_gradient2(limits=c(-700, 600), low="#f1a340", high="#998ec3") +
...
Per suggestion of a reviewer, make a cartoon figure that shows the contrasting results of the *enhance defense* and the *tolerance* hypotheses.

The *enhanced response hypothesis* posits that temperature adaptation uses existing defense mechanisms, which should result in significant overlap in response. This pattern would be apparent as an excess of transcripts with the same response pattern between species (e.g. more shared "Low" transcripts) and a deficit of transcripts that shift to other response patterns or become not responsive. 

```{r ed_cartoon}
enhanced_defense_dat <- md_sub1
enhanced_defense_dat$value <- 0
# set non-shared response to random negative value 
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Low"), "value"] <- runif(5, min=140, max=150)
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Bimodal"), "value"] <- rep(-200, 5)
# change shared response to positive
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Low" & enhanced_defense_dat$Apic == "Low"), "value"] <- 50
enhanced_defense_dat[which(enhanced_defense_dat$Acar == "Bimodal" & enhanced_defense_dat$Apic == "Bimodal"), "value"] <- 50

enhanced_defense_dat

# plot
enhanced_defense_plot <- qplot(y=Apic, x=Acar, data=enhanced_defense_dat, fill=value, geom="tile", xlim = rev(levels(enhanced_defense_dat$Acar))) + 
  scale_fill_gradient2(limits=c(-500, 500), low="#f1a340", high="#998ec3") +
  theme(axis.title = element_text(face="italic")) +
  labs(y = "A. picea", x = "A. carolinensis") +
  theme(legend.position = "right") + 
  geom_rect(xmin = 0.5, xmax = 1.5, ymin = 3.5, ymax = 4.5, fill = "transparent", linetype=2, colour="black") +
...
# title
enhanced_defense_plot <- enhanced_defense_plot + ggtitle('Enhanced Response hypothesis') + theme(plot.title = element_text(hjust=0, size = 12))
enhanced_defense_plot
```

In contrast, the *tolerance hypothesis* predicts that genes involved in active defense will become non-responsive or shift to other response categories in the better-adapted species. 

```{r tol_cartoon}
tolerance_dat <- md_sub1
tolerance_dat$value <- 0
# set non-shared response to random positive value 
tolerance_dat[which(tolerance_dat$Acar == "Low"), "value"] <- runif(5, min=140, max=160)
tolerance_dat[which(tolerance_dat$Acar == "Bimodal"), "value"] <- runif(5, min=140, max=150)
...
# plot
tolerance_plot <- qplot(y=Apic, x=Acar, data=tolerance_dat, fill=value, geom="tile", xlim = rev(levels(tolerance_dat$Acar))) + 
  scale_fill_gradient2(limits=c(-500, 500), low="#f1a340", high="#998ec3") +
  theme(axis.title = element_text(face="italic")) +
  labs(y = "A. picea", x = "A. carolinensis") +
  theme(legend.position = "none") + 
...
# title
tolerance_plot <- tolerance_plot + ggtitle('Tolerance hypothesis') + theme(plot.title = element_text(hjust=0, size = 12))
tolerance_plot

# plot together
png("results/predictions_Fig1.png")
grid.arrange(enhanced_defense_plot, tolerance_plot, ncol=2, widths = c(3.75,3))
dev.off()
```
...
```{r gen_assimilation_high}
Ap.high.Ac.nr <- interaction.response.type[which(interaction.response.type$A22.type == "High" & interaction.response.type$Ar.type == "NotResp"), ]
...
Ap.bim.Ac.nr <- interaction.response.type[which(interaction.response.type$A22.type == "Bimodal" & interaction.response.type$Ar.type == "NotResp"), ]
...
- compare degree of upregulation to constitutive expression
...
# reshape data
Ap.df <- data.frame(Transcript = rep(interaction.response.type$Transcript, times = 2), colony = rep(c("ApVT", "AcNC"), each = length(interaction.response.type$Transcript)), max.val = c(interaction.response.type$A22.max, interaction.response.type$Ar.max))
...
As with genes turned on at high temperatures, *A. carolinensis* **Intermediate** genes are down-regulated on at higher temperatures than *A. picea* **Intermediate** genes. Towards low temperatures, the mean temperature of downregulation was marginally higher for *A. carolinensis* than *A. picea*.
...
responsive.lms.ann.type <- merge(responsive.lms.ann.type, interaction.response.type[,c("Transcript", "A22.type", "Ar.type")], by = "Transcript", all = TRUE)
...
Ap.high <- interaction.response.type[which(interaction.response.type$Ar.type == "High" & interaction.response.type$A22.type == "High"), "Transcript"]
...
Ap.low <- interaction.response.type[which(interaction.response.type$Ar.type == "Low" & interaction.response.type$A22.type == "Low"), "Transcript"]
...
Ap.bim <- interaction.response.type[which(interaction.response.type$A22.type == "Bimodal" & interaction.response.type$Ar.type == "Bimodal"), "Transcript"]
...
Ap.int <- interaction.response.type[which(interaction.response.type$A22.type == "Intermediate" & interaction.response.type$Ar.type == "Intermediate"), "Transcript"]
...
Ar.int.A22.bim <- interaction.response.type[which(interaction.response.type$Ar.type == "Intermediate" & interaction.response.type$A22.type == "Bimodal"), "Transcript"]
...
Ar.int.A22.low <- interaction.response.type[which(interaction.response.type$Ar.type == "Intermediate" & interaction.response.type$A22.type == "Low"), "Transcript"]
...
```{r gsea_Ar_int_A22_lowbim, results = 'hide'}
# genes with *Intermediate* expression in Ar and *Low* in A22
...
```{r gsea_Ar_int_A22_lowbim_cluster}
# similarity among enriched GO terms
...
shiny.dt <- TPM.dt.sub[responsive.lms.ann.type[,list(Transcript, best.hit.to.nr, GO.Biological.Process)]]
...
save(TPM.dt.sub, RxNpval, sigtable, chi1, chi2, chi3, chi4, chi5, resp.TPM.dt.sub, Ap.type.table, tt2, tt3, GO.stress.Xsq, GO0006950.responsive, Ap.df, w1, w2, w3, hsp_responsive, hsp_all, mh.test, mh_plot, Ap.BP.resultParentChild, resp.stress.responsive.count, T_on.high.ttest, T_on.low.ttest, T_off.int.high.ttest, T_off.int.low.ttest, Ap.gsea.intersect, Ar.int.A22.bim.gsea, Ar.int.A22.low.gsea, Ar.int.A22.lowbim.gsea, quadratic.lms, resp.TPM.dt.sub.pred, md, file = paste("doc/ms_", Sys.Date(), ".RData", sep = ""))

save(sigtable, chi1, chi2, chi3, chi4, chi5, chi6, resp.TPM.dt.sub, Ap.type.table, tt2, tt3, GO.stress.Xsq, GO0006950.responsive, Ap.df, w1, w2, w3, hsp_responsive, hsp_all, mh.test, mh_plot, T_on.high.ttest, T_on.low.ttest, T_off.int.high.ttest, T_off.int.low.ttest, Ar.int.A22.bim.gsea, Ar.int.A22.low.gsea, Ar.int.A22.lowbim.gsea, md, file = paste("doc/ms_slim_", Sys.Date(), ".RData", sep = ""))
```
