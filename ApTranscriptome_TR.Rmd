...
keepthis <- function(lmitem, term = NA) {
  coefs <- coefficients(lmitem)
  coef.names <- names(coefs)
  keep <- ifelse(term %in% coef.names, TRUE, FALSE)
}

# identify sets of transcripts that include each term in final model
colony.transcripts <- RxNlmAIC[which(Map(keepthis, RxNlmAIC, term = "colonyAr") == TRUE)]
temp.lin.transcripts <- RxNlmAIC[which(Map(keepthis, RxNlmAIC, term = "val") == TRUE)]
temp.quad.transcripts <- RxNlmAIC[which(Map(keepthis, RxNlmAIC, term = "I(val^2)") == TRUE)]
col.by.temp.transcripts <- RxNlmAIC[which(Map(keepthis, RxNlmAIC, term = "colonyAr:val") == TRUE)]
col.by.temp.quad.transcripts <- RxNlmAIC[which(Map(keepthis, RxNlmAIC, term = "colonyAr:I(val^2)") == TRUE)]

# union of thermally-responsive transcripts
responsive.transcripts <- union(temp.lin.transcripts, temp.quad.transcripts)
responsive.transcripts <- union(responsive.transcripts, col.by.temp.transcripts)
responsive.transcripts <- union(responsive.transcripts, col.by.temp.quad.transcripts)
length(responsive.transcripts)

# transcripts that have a colony effect only 
colony.transcripts <- setdiff(RxNlmAIC, responsive.transcripts)

# transcripts that have colony x temperature interaction
interaction.transcripts <- union(col.by.temp.transcripts, col.by.temp.quad.transcripts)
...
temperature.transcripts <- setdiff(responsive.transcripts, interaction.transcripts)
```
