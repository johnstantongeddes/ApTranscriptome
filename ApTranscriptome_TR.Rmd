...
# identify responsive transcripts using robust regression
RxNout <- RxNseq(f = TPM.dt.sub, model = model, threshold = 0.05)

RxNout <- RxNseq(f = test, model = model, threshold = 0.05)


## debug
        lmrob.out345 <- suppressWarnings(eval(parse(text = paste("lmrob(", model, ", setting = 'KS2011', data = piece345)", sep = ""))))
