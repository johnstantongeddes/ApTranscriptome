...
test.transcripts <- temperature.transcripts[1:10]

response.type <- ldply(test.transcripts, .progress = "text", RxNtype)
...
RxNtype <- function(lmitem) {
  ...
  max.val
  # for each colony
  for(c in levels(pout$colony)) {
    
    
    # get max val
    max.val <- pout[which(pout$pTPM == max(pout$pTPM)), "val"]
    # get min val
    min.val <- pout[which(pout$pTPM == min(pout$pTPM)), "val"]
    ...
    if(max(pout[pout$val < 10, "pTPM"]) > (pout[pout$val == 19.5, "pTPM"] + 2*sd(pout$pTPM)) & max(pout[pout$val > 10, "pTPM"]) > (pout[pout$val == 19.5, "pTPM"] + 2*sd(pout$pTPM)))   exp_type <- "Bimodal" else {
      # expression type "High" if max expression at temperature above 30C
      if(max.val > 30) exp_type <- "High" else {
        # expression type "Low" if max expression at temperature below 10C
        if(max.val < 10) exp_type <- "Low" else
          exp_type <- "Intermediate"
        } # end else
      } # end else
    }
  }
