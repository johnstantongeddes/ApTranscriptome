...
mosaicplot(tt2, ylab = "A. picea", xlab = "", main = "", color = mosaic.colors, cex.axis = 0.01)
labels <- c("High", "Low", "Bimodal", "Intermediate", "NotResp")
text(c(.1,.3,.55,.75,.95), par("usr")[3] - 0.05, srt = 45, adj = 1, labels = labels, xpd = TRUE)
text(par("usr")[2] + 0.25, c(.95, .75, .55, .35, .1), srt = 0, adj = 1, labels = labels, xpd = TRUE)
text(.5, par("usr")[4] + .1, "A. carolinensis", xpd = TRUE)
```
