...
overlap <- c(length(Ap.response.type[which(Ap.response.type$A22.type == "Bimodal" & Ap.response.type$Ar.type == "Bimodal"), "Transcript"]),
             ...
             length(Ap.response.type[which(Ap.response.type$A22.type == "NotExp" & Ap.response.type$Ar.type == "NotExp"), "Transcript"]))

# make table
Ap.type.table <- rbind(A22.type.table, Ar.type.table, overlap)
rownames(Ap.type.table) <- c("A22", "Ar", "Both")

# re-orderfor printing
Ap.type.table <- Ap.type.table[ , c(2,4,1,3,5)]

pandoc.table(Ap.type.table, style="rmarkdown", caption = "Number of thermally-responsive transcripts with maximum expression at high, low, intermediate, both high and low (bimodal) temperatures or are not expressed for each colony and their overlap.")
...
One of two tests, for *Ar* intermediate transcripts also finds greater constitutive expression in *Ar* than *A22*. 
...
t.test(Ar.int.sd$exp_sd, A22.int.sd$exp_sd)
```
...
# t-test
t.test(varshape.out$s.offset, alternative = "two.sided")
...
# t-test
t.test(varshape.out$s.slope, alternative = "two.sided")
...
responsive.transcripts.ann <- signif.transcripts[names(responsive.transcripts)]
# GO 'response to stress' hits in responsive transcripts
responsive.transcripts.ann[grep("GO:0006950", responsive.transcripts.ann$GO.Biological.Process), list(Transcript, best.hit.to.nr)]

resp.stress.responsive.count <- nrow(responsive.transcripts.ann[grep("GO:0006950", responsive.transcripts.ann$GO.Biological.Process), list(Transcript, best.hit.to.nr)])
...
GO.stress.table <- matrix(rbind(resp.stress.responsive.count, nrow(responsive.transcripts.ann) - resp.stress.responsive.count, resp.stress.all.count, nrow(annotationtable) - resp.stress.all.count), nrow = 2)
...
```{r grep_shock}
hsp_responsive <- responsive.transcripts.ann[grep("shock", responsive.transcripts.ann$best.hit.to.nr), list(Transcript, best.hit.to.nr)]
nrow(hsp_responsive)
...
# Run GSEA
Ap.high.gsea <- gsea(genelist = Ap.geneList.high, geneID2GO = geneID2GO)
...
pandoc.table(A22.low.gsea)
```


**Ar 'Low' genes not in Ar**
...
pandoc.table(A22.bim.gsea)
```


**Ar 'Bimodal' genes not in Ar**
...
# Run GSEA
Ar.int.gsea <- gsea(genelist = Ar.geneList.int, geneID2GO = geneID2GO, plotpath = "results/Ar_int_BP_topGO_sig_nodes.pdf")
pandoc.table(Ar.int.gsea)
```
