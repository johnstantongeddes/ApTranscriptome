...
A22.high <- Ap.response.type[which(Ap.response.type$A22.type == "High"), ]

# Compare expression at optimum temp (A22.opt) between colonies using t-test
t.test(A22.high$A22.opt, A22.high$Ar.opt)
boxplot(A22.high$A22.opt, A22.high$Ar.opt)

# T test on log-transformed values to control for outliers
t.test(log(A22.high$A22.opt+1), log(A22.high$Ar.opt+1))
boxplot(log(A22.high$A22.opt+1), log(A22.high$Ar.opt+1))
```
...
```{r A22_high_wilcoxon}
w1 <- wilcox.test(A22.high$A22.opt, A22.high$Ar.opt, alternative = "two.sided", paired = TRUE, conf.int = TRUE)
w1
```
...
Ar.low <- Ap.response.type[which(Ap.response.type$Ar.type == "Low"), ]

# t-test with log values
t.test(log(Ar.low$A22.opt+1), log(Ar.low$Ar.opt+1))
boxplot(log(Ar.low$A22.opt+1), log(Ar.low$Ar.opt+1))

# Wilcoxon signed rank-test
w2 <- wilcox.test(Ar.low$A22.opt, Ar.low$Ar.opt, alternative = "two.sided", paired = TRUE, conf.int = TRUE)
w2
```
...
```{r ggplot_high}
trp_A22_high <- ggplot(resp.TPM.dt.sub[A22.high.transcripts], aes(x=val, y=TPM.scaled, group=Transcript)) + 
  geom_smooth(method = "lm", formula = y ~ poly(x, 2)) + 
  facet_grid(. ~ colony2) + 
  scale_y_continuous(name="Expression (scaled)") +
  scale_x_continuous(name=expression(paste("Temperature ", degree, "C")))
