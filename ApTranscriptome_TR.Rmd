...
```{r gene_overlap}
# transcripts that are low in A22 and intermediate in Ar
A22.low.Ar.int <- Ap.response.type[which(Ap.response.type$A22.type == "Low" & Ap.response.type$Ar.type == "Intermediate"), "Transcript"]
A22.low.Ar.int.lms <- responsive.lms[A22.low.Ar.int]


# transcripts that are low in both A22 and Ar
A22.low.Ar.low <- Ap.response.type[which(Ap.response.type$A22.type == "Low" & Ap.response.type$Ar.type == "Low"), "Transcript"]
A22.low.Ar.low.lms <- responsive.lms[A22.low.Ar.low]


head(int.low.overlap)

# compare linear and quadratic model terms
...
A22.low.Ar.int.TPM.dt.sub <- resp.TPM.dt.sub.pred[J(intersect(A22.low, Ar.int), "A22")]
dim(A22.low.Ar.int.TPM.dt.sub)

l5 <- length(unique(A22.low.Ar.int.TPM.dt.sub$Transcript))

A22.low.Ar.int.T_on <- data.frame(Transcript = unique(A22.low.Ar.int.TPM.dt.sub$Transcript), colony = rep("ApVT", length = l5), type = rep("High", length = l5), T_on = NA)

# of these, how many have greatest change in expression as a decrease at high temperatures?
for(i in unique(A22.low.Ar.int.TPM.dt.sub$Transcript)) {
    subdf <- A22.low.Ar.int.TPM.dt.sub[i]
    T_on <- subdf[median(which(diff(subdf$TPM) == max(diff(subdf$TPM))))+1, val]
    A22.low.Ar.int.T_on[which(A22.low.Ar.int.T_on$Transcript == i), "T_on"] <- T_on
}   
...
save(RxNpval, sigtable, chi1, resp.TPM.dt.sub, tt2, tt3, GO.stress.Xsq, GO0006950.responsive, Ap.df, w1, w2, w3, Ap.T_on, Ap.BP.resultParentChild, Ap.gsea1, Ap.setdiff.gsea, resp.stress.responsive.count, t.mean, t.slope, t.curvature, t.wiggle, t.themsens, t.thermbreadth, file = "ms.RData")
```
