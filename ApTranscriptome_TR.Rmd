...
rownames(Ap.type.table) <- c("A22", "Ar")

pandoc.table(Ap.type.table, style="rmarkdown", caption = "Number of transcripts with maximum expression at high, low, intermediate or both high and low (bimodal) temperatures.")

# only transcripts that differ in expression by colony
A22.interaction.type.table <- table(interaction.response.type[ , 'A22.type'])
Ar.interaction.type.table <- table(interaction.response.type[ , 'Ar.type'])

Ap.interaction.type.table <- rbind(A22.interaction.type.table, Ar.interaction.type.table)
rownames(Ap.interaction.type.table) <- c("A22", "Ar")

pandoc.table(Ap.interaction.type.table, style="rmarkdown", caption = "For transcripts that differ in expression pattern between *A22* and *Ar*, the number of transcripts with maximum expression at high, low, intermediate or both high and low (bimodal) temperatures.")
...
Probability distribution function of peak expression for transcripts that differ in expression between *A22* and *Ar*

```{r max_exp_PDF}
# reshape data
Ap.df <- data.frame(Transcript = rep(Ap.response.type$Transcript, times = 2), colony = rep(c("A22", "Ar"), each = length(Ap.response.type$Transcript)), max.val = c(Ap.response.type$A22.max, Ap.response.type$Ar.max))
...
for(i in 1:nrow(varshape.out)) {
  varshape.out[i, "s.slope"] <- (varshape.out[i, "offset"] / mean(c(varshape.out[i, "A22.slope"], varshape.out[i, "Ar.slope"])))
}
...
for(i in 1:nrow(varshape.out)) {
  varshape.out[i, "s.curvature"] <- (varshape.out[i, "offset"] / mean(c(varshape.out[i, "A22.curve"], varshape.out[i, "Ar.curve"])))
}
...
# change 'Inf' values to 0
varshape.out[which(varshape.out$Ar.curve == 0 & varshape.out$A22.curve == 0), "s.wiggle"] <- 0
# note that only 407 transcripts have any 'wiggle'
length(which(!is.na(varshape.out$s.wiggle)))
...
# table
pandoc.table(varshape.out[which(abs(varshape.out$offset)>1000), c(".id", "A22.mean", "Ar.mean", "offset")])

# t test
t.test(varshape.out$A22.mean, varshape.out$Ar.mean, paired = TRUE)
t.test(log(varshape.out$A22.mean), log(varshape.out$Ar.mean), paired = TRUE)

wilcox.test(varshape.out$A22.mean, varshape.out$Ar.mean, alternative = "two.sided", paired = TRUE, conf.int = TRUE)

t.test(varshape.out$A22.slope, varshape.out$Ar.slope)
t.test(varshape.out$A22.curve, varshape.out$Ar.curve)
t.test(varshape.out$A22.wiggle, varshape.out$Ar.wiggle)
...
Use `selectTranscript` function to select transcripts based on 'A22.exp.type'. As only significant transcripts have an 'A22.exp.type' assigned, this is a small subset of the above.

```{r A22.exp.type_function}
selectTranscript <- function(score) {
    return(score == 1)
}

# To select genes, add 'A22.exp.type' to RxN df
RxNout.t <- merge(RxNout, Ap.A22.exp.type, by="Transcript", all=TRUE)
length(which(!is.na(RxNout.t$A22.exp.type)))
```
...
Ap.bim.geneList[which(RxNout.t$A22.exp.type=="Bimodal")] <- 1
...
Ap.int.geneList[which(RxNout.t$A22.exp.type=="Intermediate")] <- 1
...
Ap.hig.geneList[which(RxNout.t$A22.exp.type=="High")] <- 1
...
Ap.low.geneList[which(RxNout.t$A22.exp.type=="Low")] <- 1
