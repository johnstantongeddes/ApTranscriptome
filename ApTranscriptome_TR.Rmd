...
Ap.induction <- data.table(Ap.induction)
setkey(Ap.induction, Transcript)
Ap.response.type2 <- Ap.induction[Ap.response.type]


# plot constitutive expression vs induction for High, Low and Bimodal genes only

setkey(Ap.response.type2, A22.type)
Ap.response.type2.up <- Ap.response.type2[A22.type!="Intermediate", ]
setkey(Ap.response.type2.up, Ar.type)
Ap.response.type2.up <- Ap.response.type2.up[Ar.type!="Intermediate", ]


gg5 <- ggplot(Ap.response.type2.up, aes(x = log(A22.opt), y = log(upreg), colour = A22.type)) + 
  geom_point() + 
  facet_grid(. ~ colony) 
