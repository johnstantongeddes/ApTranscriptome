...
The *enhanced defense hypothesis* posits that temperature adaptation uses existing defense mechanisms, which should result in significant overlap in response. This pattern would be apparent as an excess of transcripts with the same response pattern between species (e.g. more shared "Low" transcripts) and a deficit of transcripts that shift to other response patterns or become not responsive. 
...
# plot
enhanced_defense_plot <- qplot(x=Apic, y=Acar, data=enhanced_defense_dat, fill=value, geom="tile", ylim = rev(levels(enhanced_defense_dat$Acar))) + 
...
# title
enhanced_defense_plot <- enhanced_defense_plot + ggtitle('Enhanced defense hypothesis') + theme(plot.title = element_text(hjust=0))
...
# plot
tolerance_plot <- qplot(x=Apic, y=Acar, data=tolerance_dat, fill=value, geom="tile", ylim = rev(levels(tolerance_dat$Acar))) + 
  scale_fill_gradient2(limits=c(-500, 500), low="#f1a340", high="#998ec3") +
  theme(axis.title = element_text(face="italic")) +
  labs(x = "A. picea", y = "A. carolinensis") +
  theme(legend.position = "bottom") + 
...
grid.arrange(enhanced_defense_plot, tolerance_plot, ncol=1, heights = c(0.45, 0.6))
dev.off()
```
